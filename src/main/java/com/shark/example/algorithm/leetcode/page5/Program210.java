...
public class Program210 {
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        //generate graph and root
        HashMap<Integer, Set<Integer>> graph = generateGraph(prerequisites);
//        System.out.println("graph: " + new Gson().toJson(graph));
        ...
        int orderIndex = 0;
        while (!task.isEmpty()) {
            int course = task.remove(0);
            //check
            if (!checked.contains(course)) {
                boolean valid = checkCourse(course, graph, checked, new HashSet<>());
                if (!valid) {
                    return new int[0];
                }
            }
            boolean isRequestsAdded = checkRequestAdded(course, graph, added);
//            System.out.println("task course: " + course + ", isRequestsAdded: " + isRequestsAdded);
            if (isRequestsAdded) {
    ...
    private HashMap<Integer, Set<Integer>> generateGraph(int[][] prerequisites) {
        HashMap<Integer, Set<Integer>> graph = new HashMap<>();
        for (int[] preRequest : prerequisites) {
            int course = preRequest[0];
            int request = preRequest[1];
            Set<Integer> requestSet = graph.getOrDefault(course, new HashSet<>());
            requestSet.add(request);
            graph.put(course, requestSet);
        }
        return graph;
    }

    private boolean checkCourse(int course,
                                HashMap<Integer, Set<Integer>> graph, Set<Integer> checkedSet, Set<Integer> ancestorSet) {
//        System.out.println("checkCourse course: " + course);
        if (ancestorSet.contains(course)) {
            return false;
        }

        Set<Integer> requestSet = graph.get(course);
        if (requestSet != null) {
//            System.out.println("checkCourse requestSet: " + requestSet + ", ancestorSet: " + ancestorSet);
            for (Integer request : requestSet) {
                if (checkedSet.contains(request)) {
                    continue;
                }
                Set<Integer> requestAncestorkSet = new HashSet<>(ancestorSet);
                requestAncestorkSet.add(course);

                boolean valid = checkCourse(request, graph, checkedSet, requestAncestorkSet);
                if (!valid) {
                    return false;
                }
            }
        }
        checkedSet.add(course);
        return true;
    }

    public boolean checkRequestAdded(int course, HashMap<Integer, Set<Integer>> graph, Set<Integer> added) {
        Set<Integer> requestSet = graph.get(course);
        if (requestSet != null) {
            for (Integer request : requestSet) {
                if (!added.contains(request)) {
                    return false;
                }
            }
        }
...
//        int[][] prerequisites = new int[][]{{1, 0}, {2, 0}, {3, 1}, {3, 2}};
        int numCourses = 10;
        int[][] prerequisites = new int[][] {{5,8},{3,5},{1,9},{4,5},{0,2},{7,8},{4,9}};
...
//        int[][] prerequisites = new int[][] {{1, 0}, {2, 6}, {1, 7}, {5, 1}, {6, 4}, {7, 0}, {0, 5}};
        int[] order = new Program210().findOrder(numCourses, prerequisites);
