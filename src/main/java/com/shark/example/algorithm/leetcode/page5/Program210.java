...
public class Program210 {
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for (int[] prerequisite : prerequisites) {
            ...
            int request = prerequisite[1];
            rootSet.remove(course);
            Set<Integer> childrenSet = graph.getOrDefault(request, new HashSet<>());
            childrenSet.add(course);
            graph.put(request, childrenSet);
            if (course > maxCourse) {
                maxCourse = course;
            }
            if (request > maxCourse) {
                maxCourse = request;
            }
        }
        System.out.println("rootSet: " + rootSet);
        System.out.println("graph: " + new Gson().toJson(graph));
        if (maxCourse >= numCourses) {
            return new int[0];
        }

        List<Integer> rootList = new ArrayList<>(rootSet);
        rootList.sort(Collections.reverseOrder());

        Set<Integer> addSet = new HashSet<>();
        List<List<Integer>> childResultList = new ArrayList<>();
        for (Integer root : rootList) {
            Set<Integer> checkSet = new HashSet<>();
            List<Integer> nodeResult = generateNodeOrderResult(root, checkSet, addSet, graph);
            if(nodeResult.isEmpty()) {
                return new int[0];
            }
            childResultList.add(nodeResult);
        }
        List<Integer> resultList = combineResult(childResultList);
    ...
    public List<Integer> generateNodeOrderResult(
            int node, Set<Integer> checkSet, Set<Integer> addSet, Map<Integer, Set<Integer>> graph) {
        List<Integer> result = new ArrayList<>();
        result.add(node);
        addSet.add(node);
        Set<Integer> childSet = graph.get(node);
        List<List<Integer>> childResultList = new ArrayList<>();
        if(childSet != null && !childSet.isEmpty()) {
            checkSet.add(node);
            List<Integer> childList = new ArrayList<>(childSet);
            ...
            for (Integer child : childList) {
                ...
                if (addSet.contains(child)) {
                    continue;
                }
                Set<Integer> childCheckSet = new HashSet<>(checkSet);
                List<Integer> childResult = generateNodeOrderResult(child, childCheckSet, addSet, graph);
        ...
        result.addAll(combineResult(childResultList));
        return result;
    }

    public List<Integer> combineResult(List<List<Integer>> resultList) {
        List<Integer> combineResult = new ArrayList<>();
        ...
        while (hasNext) {
            ...
            for(List<Integer> result : resultList) {
                if(index < result.size()) {
                    tempResult.add(result.get(index));
                    hasNext = true;
                }
            }
            combineResult.addAll(tempResult);
            index++;
        }
